...
function DynamicCard({ width = 100, height = 110, label, image, title, mediaLabel }) {
  ...
  return (
    ...
    >
      ...
      {showLabel && (
        <Box sx={{ position: 'absolute', top: 10, right: 10, backgroundColor: 'rgba(255,255,255,0.2)', px: 1, py: 0.2, borderRadius: 1, zIndex: 2 }}>
          <Typography variant="caption" sx={{ color: 'white', fontSize: '0.7rem' }}>{label}</Typography>
        </Box>
      )}
...
export default function Page() {
  ...
  return (
    <>
      ...
      <Box sx={{ minHeight: '100vh', backgroundColor: '#f5f5f5', display: 'flex', justifyContent: 'center', alignItems: 'center', px: 2, py: 6 }}>
        <LayoutGroup>
          <Box
            ref={containerRef}
            sx={{ position: 'relative', width: '100%', maxWidth: '1800px', aspectRatio: `${COLS}/${ROWS}`, userSelect: 'none' }}
          >
